...
class ada_split_test(object):
    ...
    Parameters
    ----------
    inf_feats: array-like | shape = (num of hypothsized feats sets, num of samples, 4)
     Array of hypothsized feats tests under hypothesis testings, each array many lists each of which consists 4 indexs of each picture's  region boundary.
     If there is only one feature you want to test, and there are n pictures, the array should be:
      [[left_0, right_0, up_0, down_0], [left_1, right_1, up_1, down_1], ..., [left_n, right_n, up_n, down_n]]
     If there are m hypothsized feats tests (m>1) you want to test, and there are n. pictures, the array should be:
      [[[left_00, right_00, up_00, down_00], [left_01, right_01, up_01, down_01], ..., [left_0n, right_0n, up_0n, down_0n]],
       [[left_10, right_10, up_10, down_10], [left_11, right_11, up_11, down_11], ..., [left_1n, right_1n, up_1n, down_1n]],
       ...,
       [[left_m0, right_m0, up_m0, down_m0], [left_m1, right_m1, up_m1, down_m1], ..., [left_mn, right_mn, up_mn, down_mn]]]
    ...
    def __init__(self, model, model_mask, change='mask', alpha=.05, verbose=0, loss='MeanSquaredError', cp_path='./save'):
        self.model = model
    ...
    def metric(self, y_true, y_pred):
        ...
        if type(self.loss) == str:
            loss = getattr(tf.keras.losses, self.loss)(from_logits=False, reduction=tf.keras.losses.Reduction.NONE)
        else: 
            loss = self.loss
    ...
    def reload_model(self, path_full, path_mask):
        ...
        ----------
        path_full:
         The path to reload the full model we saved before.
         
        path_mask:
         The path to reload the masked model we saved before.
        """
    ...
    def dual_feat(self, X, inf_feats):
        """
        ...
        inf_feats: array-like
         The region to be masked or permuted.

        """
        if self.change == 'mask':
            Z = self.mask_cov(X, inf_feats=inf_feats)
        elif self.change == 'perm':
            Z = self.perm_cov(X, inf_feats=inf_feats)
        return Z
    ...
    def mask_cov(self, X, inf_feats):
        ...
        ## median can work for both continuous and categorical data type
        Z_median = np.percentile(Z, 50, method='nearest', axis=0)

        if len(np.array(inf_feats).shape)==3:
            for i in range(np.array(inf_feats).shape[0]):
                for j in range(n_sample):
                    [left, right, up, down] = [inf_feats[i][j,k] for k in range(4)]
                    try:
                        Z[j,up:down+1,left:right+1,0] = Z_median[up:down+1,left:right+1,0]
                    except:
                        Z[j,up:down+1,left:right+1,0] = 0                 
                    
        elif len(np.array(inf_feats).shape)==2:
            for i in crange(n_sample):
                [left, right, up, down] = [inf_feats[i,k] for k in range(4)]
                try:
                    Z[i, up:down+1, left:right+1, 0] = Z_median[up:down+1,left:right+1,0]
                except:
    ...
    def perm_cov(self, X, inf_feats):
        ...
        if len(np.array(inf_feats).shape) == 3:
            for i in range(np.array(inf_feats).shape[0]):
                for j in range(n_sample):
                    [left, right, up, down] = [inf_feats[i][j,k] for k in range(4)]
                    Z[j,up:down+1,left:right+1,0] = Z_perm[j,up:down+1,left:right+1,0]
                    
        elif len(np.array(inf_feats).shape) == 2:
            for i in range(n_sample):
                [left, right, up, down] = [inf_feats[i,j] for j in range(4)]
                Z[i,up:down+1,left:right+1,0] = Z_perm[i,up:down+1,left:right+1,0]
    ...
    def noise_cov(self, X, inf_feats):
        ...
        if len(np.array(inf_feats).shape)==3:
            for i in range(np.array(inf_feats).shape[0]):
                for j in range(n_sample):
                    [left, right, up, down] = [inf_feats[i][j,k] for k in range(4)]
                    dim = Z[j,up:down+1,left:right+1,0].shape
                    Z[j,up:down+1,left:right+1,0] = np.random.randn(dim[0], dim[1])
                                       
        elif len(np.array(inf_feats).shape)==2:
            for i in range(n_sample):
                [left, right, up, down] = [inf_feats[i,j] for j in range(4)]
                dim = Z[j,up:down+1,left:right+1,0].shape
                Z[i,up:down+1,left:right+1,0] = np.random.randn(dim[0], dim[1]) 

        return Z

    def hp_tuning(self, X, y, Z, fit_params={}, split_params={}):
        """
        ...
        inf_feats: array-like | shape = (num of features, num of samples, 4)
            The region under hypothesis testing.
        ...
        if (Z is None) and (inf_feats is None):
            raise ValueError('Z and inf_feats, at least one of them should be given!')
        elif (Z is not None) and (inf_feats is not None):
            warnings.warn("Both Z and inf_feats are provided, inf_feats will be ignored!")
        elif (Z is None) and (inf_feats is not None):
            Z = self.dual_feat(X, inf_feats)
        else:
            pass
        ...
        if split == 'one-split':
            ...
            for perturb_idx_tmp in range(-perturb_scale, perturb_scale):
                ...
                for ratio_tmp in ratio_grid:
                    ...
                    p_value = []
                    for h in range(split_params['cv_num']):
                        p_value_cv = []
                        self.reset_model()
                
                        # generate permutated samples, where the null hypothesis is correct
                        X_perm = X.copy()
                    ...
                    p_value_cp = np.array([comb_p_value(p_value[:,i], cp=split_params['cp']) for i in range(num_perm)])
                    # calculate the err1
    ...
    def testing(self, X, Z, y, inf_ratio=None, perturb=None, fit_params={}, split_params={}, cv_num=5, cp='hommel'):
        """
        ...
        split_params: {dict of splitting parameters}
            ...
            ratio_method: {'fuse', 'log'}, default = 'fuse'
        ...
        if split_params['ratio_method'] == 'log':
        ...
        for h in range(split_params['cv_num']):
            ...
            pred_y_mask = self.model_mask.predict(Z_inf_mask)
        ...
        p_value_mean = comb_p_value(P_value_cv, cp=split_params['cp'])
