...
for i in range(100):
	...
	d, L = d0, L0
	model = Reg_model(p=p, d=d, L=L, optimizer=SGD(lr=.001))
	es = EarlyStopping(monitor='val_loss', mode='min', verbose=verbose, patience=20, restore_best_weights=True)

	est_n, est_m = funs.adaRatio(model=model, X=X, y=Y, es=es, init_size=.01, num_grid=50, gap=10, 
								ratio=[.8, .1, .1], plot=True, verbose=verbose)
	print('the number of estimated training sample: %d; number of inference sample: %d' %(est_n, est_m))
	## training
	...
	history = model.fit(X_train, y_train, epochs=100, validation_split=.1, verbose=verbose, callbacks=[es])
	...
	for inf_cov in [range(0, 5), range(3, 8), range(45, 50), range(95, 100)]:
		...
		history_mask = model_mask.fit(Z_train, y_train, epochs=100, validation_split=.1, verbose=verbose, callbacks=[es])
...
print('training sample: %d; MSE: %.3f, MSE_mask: %.3f' %(est_n, SE_lst.mean(), SE_mask_lst.mean()))
